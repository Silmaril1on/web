...
const ParticleImage = ({ imageSrc }) => {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const gold = new THREE.Color("#ff9900");
      const cream = new THREE.Color("#fcf5df");
      ...
      for (let y = 0; y < canvas.height; y += step) {
        for (let x = 0; x < canvas.width; x += step) {
          ...
          if (alpha > 128) {
            ...
            const color = brightness > 0.5 ? gold : cream;
...
const ParticleAnimation = ({ logoSrc = "/assets/elivagar.png" }) => {
  return (
    <motion.div
      initial={{ y: "500px" }}
      animate={{ y: 0 }}
